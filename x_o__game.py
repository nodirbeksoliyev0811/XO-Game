...
class TTT:
	def __init__(self):
		print(Fore.GREEN+' Easy   - 1\n',Fore.YELLOW+'Medium - 2\n',Fore.RED+'Hard   - 3')
		self.mode = input(Fore.WHITE+'Choose: ')
		self.sign= print(Fore.RED+'X',Fore.WHITE+'or',Fore.BLUE+'O',Fore.WHITE+': ',end='')
		self.sign = input()
		...
		self.step = 0
		if self.mode == '1':
			print(Fore.GREEN+'   Easy'),sleep(0.5),self.start(self.put)
	def start(self,f):
		qadam = 1
		...
		while qadam<=9:
			...
			self.show()
			if self.step:
				if f(int(input(Fore.WHITE+'Choose a number: '))-1):
					qadam+= 1
			...
			if res[0]:
				system('clear')
				self.show()
				if self.sign in res[1]:
					print(Fore.RED+'  !You win!')
					playsound('x.mp3')
				else:
					print(Fore.BLUE+'   !I win!')
					playsound('o.mp3')
				break
	...
	def show(self):
		...
		for i in range(9):
			l = len(self.desk[i])
			print(Fore.YELLOW+f"|{' '*(2-l)}",end='')
			if self.desk[i] == 'X':
				print(Fore.RED+'X ',end='')
			elif self.desk[i] == 'O':
				print(Fore.BLUE+'O ',end='')
			else:
				print(Fore.BLACK+f'{self.desk[i]} ',end='')
			if i%3 == 2:
				print(Fore.YELLOW+f"|\n{line}")
...
class TTTMedium(TTT):
	def __init__(self):
		super().__init__()
		if self.mode == '2':
			print(Fore.YELLOW+'   Medium'),sleep(0.5),self.start(self.put2)
	def put2(self,i):
...
class TTTHard(TTTMedium):
	def __init__(self):
		super().__init__()
		if self.mode == '3':
			print(Fore.RED+'   Hard'),sleep(0.5),self.start(self.put3)
	def put3(self,i):
		if self.step%2 and self.desk[i] not in 'XO':
			self.desk[i] = self.sign
			self.spaces.remove(i)
